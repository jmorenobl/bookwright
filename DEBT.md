...
### DEBT-011 — `character_presence` marca el primer término tras una comilla/raya-líder distinta de la raya de diálogo (`«`, `"`, `―` U+2015)
- **Estado:** abierta
- **Detectada en:** auditoría de `spec-041` (2026-06-22) — al cerrar DEBT-009 (la raya de diálogo `—`/`–`) se verificó **empíricamente** que la *misma clase* de fallo persiste para otros marcadores líderes tipográficos.
- **Ubicación:** `src/bookwright/io/prose.py` (`_normalize` retira encabezados ATX, viñetas/citas ASCII y —tras 041— la raya de diálogo `—`/`–`; NO retira la comilla angular `«`/`»` U+00AB/BB, las comillas tipográficas `"`/`"` U+201C/D, las comillas rectas ASCII `"`/`'`, ni la barra horizontal `―` U+2015); consumido por `character_presence._is_sentence_initial` (`_SENTENCE_END` ya cubre `¿¡` pero no estas comillas/barra).
- **Clase de deuda:** la MISMA que DEBT-008/DEBT-009 / issue #1 — acoplamiento a un marcador de superficie líder no normalizado que el heurístico no ve.
- **Descripción:** `«Esto es el porvenir»`, `"Hola"` y `―Esto` (barra horizontal U+2015) dejan el primer término citado/hablado (`Esto`, `Hola`) en offset ≠ 0 con un prefijo (`«`/`"`/`―`) que no está en `_SENTENCE_END`, así que `_is_sentence_initial` devuelve `False` y el demostrativo/saludo se marca como nombre propio sin entrada en la bible — idéntico al fallo de DEBT-009, pero para marcadores que la iteración 041 deja fuera de scope. Verificado en la auditoría de spec-041: las tres formas producen el flag espurio hoy.
- **Por qué se difiere:** 041 cierra el marcador *raya de diálogo* (la convención española dominante y el caso observado en el dogfood de `tiny-historical`). Las comillas son un marcador DISTINTO con semántica de par (apertura `«`…cierre `»`), pueden aparecer a mitad de línea como contenido citado, y su normalización (¿retirar solo la comilla de apertura líder?, ¿la de cierre?, interacción con el `¿¡` que `_SENTENCE_END` ya trata) es una decisión de diseño propia, mayor que añadir un code-point a la clase de caracteres de la raya. La barra `―` U+2015 es una variante de raya rara sin ocurrencia observada; se agrupa aquí para resolver toda la familia de marcadores líderes de una vez.
- **Resolución sugerida / versión objetivo:** una iteración futura (horizonte demand-pulled, tras 042). En el seam (`io/prose.py`), extender `_normalize` para retirar el marcador líder de cita/raya restante: `―` U+2015 como tercer code-point de raya, y un marcador de comilla de apertura líder (`«`, `"`, `"`, `'`) — restaurando el primer término a inicio-de-frase y heredando la exención existente, mismo mecanismo que 041. Ningún validador se toca.
