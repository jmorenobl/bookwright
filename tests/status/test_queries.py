...
def test_validation_summary_surfaces_not_evaluated_sorted(tmp_path: Path) -> None:
    # An empty project (no constitution, no prose, no roster) leaves four validators
    ...
    # character_unknown_mentions is ALWAYS dormant (it abstains unconditionally).
    root = write_project(tmp_path / "novel")
    context = load_context(root)
    ...
    assert "character_unknown_mentions" in names
