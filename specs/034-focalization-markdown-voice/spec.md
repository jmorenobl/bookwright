...
- **All five voice-bearing fixtures use the scaffold's markdown-prefixed form** (`- **Voz narrativa**: …`), so `focalization` is dormant on every one today and wakes on all of them after the fix. Their oracle impact has been determined precisely, not left open:
  - `tiny-historical` (third-person-limited, focal "Elena Vidal") — its oracle is the candidate to shift: `test_orchestration_workflow.py` asserts the project-wide `validation.counts` from `expected-status.md` (`{error:1, warning:6}`). The awake count is read from the validator during implementation, not back-fitted. **Verified outcome: the count is unchanged** — the now-awake `focalization` produces 0 findings on this fixture's clean third-person prose (no first-person outside dialogue, no interiority verb on non-focal "Tomás Arnela"), so the oracle stays at `{error:1, warning:6}` honestly.
