...
to enable independent implementation and validation. The only `src/bookwright/`
change is the corrective swap command `integration use` (plus
`Manifest.set_integration`), added because FR-008's original `init --here --force`
mechanism is forbidden by the ratified FR-028 (see T011 / the spec's 2026-06-03
swap-mechanism correction); otherwise this is consolidation, not feature work.
...
- [X] T004 Implement the shared E2E harness in `tests/conftest.py` (root): a `copy_fixture(name, tmp_path)` helper using `shutil.copytree` (D2, R1 — never writes outside `tmp_path`) and a `cli` fixture returning a `typer.testing.CliRunner` bound to `bookwright.cli:app` (D1). Add `tests/e2e/conftest.py` re-exporting/extending these for the `tests/e2e/` package.
...
- [X] T011 [P] [US2] Implement `tests/e2e/test_integration_swap.py` (C3, FR-008): `init --integration claude` in `tmp_path` (skills under `.claude/skills/`) → `bookwright integration use generic` (exit 0) → assert a valid `SKILL.md` set under `.agents/skills/` and that the manifest `[integration]` now records `generic`. Explicitly make NO assertion about removal of the old `.claude/skills/` dir (clarified 2026-06-03; spec Assumptions). ≤ 500 lines. **Swap-mechanism correction (2026-06-03)**: the original `init --here --force` re-init is impossible (iteration-4 FR-028 refuses re-init even with `--force`, and `init` ignores the manifest `[integration]`), so the swap is the dedicated `bookwright integration use <key>` command added this iteration (`src/bookwright/commands/integration/use.py` + `Manifest.set_integration`), with unit coverage in `tests/commands/integration/test_use.py`.
...
- [X] T012 [US3] Create `mkdocs.yml` at the repo root: `site_name`, `theme: {name: material}`, `strict: true`, and `nav` covering Inicio/Primeros pasos/Arquitectura/Comandos/Validación/Extender/FAQ (D5, docs-site contract). (Authored alongside the pages — keep nav targets in sync with T013–T019.)
- [X] T013 [P] [US3] Write `docs/index.md` — qué es Bookwright + value prop (Spanish)
- [X] T014 [P] [US3] Write `docs/getting-started.md` — install + 5-minute quickstart whose commands/flags match the shipped CLI (FR-015; mirrors `README.es.md`)
- [X] T015 [P] [US3] Write `docs/architecture.md` — curated Spanish summary that LINKS `bookwright-design.md § N.M` and does NOT duplicate it wholesale (FR-013, VR-12)
- [X] T016 [P] [US3] Write `docs/commands/` — one page or clearly delineated section per shipped executable leaf command: `init`, `check`, `version`, `validate`, `graph build`, `graph query`, `integration use` (the last is the corrective swap command added this iteration — 2026-06-03 swap-mechanism correction; FR-012, DOC-2)
- [X] T017 [P] [US3] Write `docs/validation.md` — the 4 built-in validators (`character_presence`, `focalization`, `setting_continuity`, `temporal`) + how to add a custom validator
- [X] T018 [P] [US3] Write `docs/extending.md` — new integration / custom validator / vocabulary (mirrors CONTRIBUTING)
- [X] T019 [P] [US3] Write `docs/faq.md` — common questions
- [X] T020 [US3] Finalize `README.es.md` (canonical: qué es / install / 5-min quickstart / docs links; status updated to v0.1.0) and trim `README.md` to a short English pointer to `README.es.md` + docs (FR-010, VR-15; do not regress existing content)
- [X] T021 [US3] Implement the docs↔CLI drift test (D4, VR-11, DOC-2) in `tests/e2e/test_docs_commands_match.py`: introspect `bookwright.cli:app`, **descending into registered sub-`Typer` groups to yield leaf command paths** (`init`, `check`, `version`, `validate`, `graph build`, `graph query`, `integration use` — descending into both the `graph` and `integration` groups, not the bare top-level groups), and assert **(a)** the documented command set under `docs/commands/` equals that leaf set (no missing, no extra) **and (b)** for each leaf command, every CLI option name it exposes — introspected from the command's params, excluding the auto-generated `--help` — appears in that command's documentation page (closing the FR-015 "flags" gap so an undocumented or stale flag fails CI rather than relying on author diligence). Do NOT hard-code the command or flag lists in the test — derive both from the live Typer command objects (single source of truth). Depends on T016
- [X] T022 [US3] Run `uv run --group docs mkdocs build --strict` and confirm exit 0 with zero warnings (DOC-1, FR-014, SC-004). Depends on T012–T020
...
- [X] T023 [P] [US4] Finalize `CHANGELOG.md` — add a `## [0.1.0]` entry (Keep-a-Changelog) enumerating every shipped feature, consolidating `[Unreleased]` + iterations 1–11 (CLI: `init`, `graph build/query`, `validate`, `check`, `version`; the 10 authoring commands; GOLEM model; indexer; integrations; validation system) (FR-016, VR-14, SC-008)
- [X] T024 [P] [US4] Finalize `CONTRIBUTING.md` — add sections for: create a new integration, create a custom validator, create a vocabulary (current draft lacks these) (FR-017, SC-008)
- [X] T025 [P] [US4] Verify `LICENSE` is Apache-2.0 and referenced from `pyproject.toml` (`license = "Apache-2.0"` already set) (FR-018, SC-008)
- [X] T026 [US4] Extend `.github/workflows/tests.yml`: add a docs-build gate `uv run --group docs mkdocs build --strict` (CI-5, FR-021) and an artifact-build step `uv build` producing wheel + sdist (CI-6, FR-022), then assert the built wheel bundles the packaged templates — check that `bookwright/resources/` is present in the wheel contents (`python -m zipfile -l dist/bookwright_cli-*.whl | grep -q 'bookwright/resources/'`) so a resources-less wheel fails the gate instead of failing silently at first `init` (data-model E4 "wheel includes `resources/`", SC-007); leave the existing pytest/ruff/mypy matrix gates intact (CI-1…CI-4)
- [X] T027 [US4] Document the manual packaged-install validation in this iteration's flow (D7, MAN-1…MAN-3): `uv build` → `pipx install ./dist/bookwright_cli-*.whl` → run the README quickstart against the installed CLI without touching the source tree; optionally add a `@pytest.mark.manual` test (deselected by default, exempt from coverage — R3) capturing the steps
- [X] T028 [US4] Run the full quality-gate suite locally and confirm all green (VR-13, SC-005, SC-006): `uv run pytest` (coverage **≥ 80%**, fail-closed with no round-up via `[tool.coverage.report] precision = 2` set in T001), `uv run ruff check .`, `uv run ruff format --check .`, `uv run mypy`, `uv run pre-commit run --all-files`, `uv run --group docs mkdocs build --strict`, `uv build`
...
- [X] T029 [P] Confirm every new test file (`tests/fixtures/test_fixtures.py`, `tests/e2e/*.py`) is ≤ 500 lines (Principle IV, VR-8); split if any exceeds
- [X] T030 Execute the maintainer `quickstart.md` walkthrough end-to-end and tick every "Done criteria" box (fixtures valid & counts 3/2/5; E2E green & counted; docs strict-clean with 7 areas; CHANGELOG/CONTRIBUTING/LICENSE; all gates green & coverage ≥ 80%; wheel installs and quickstart runs)
...
- **Minimal `src/bookwright/` change** — this iteration is consolidation
  (fixtures, tests, docs, metadata, CI), plus the one corrective swap command
  `integration use` (+ `Manifest.set_integration`) that makes FR-008 achievable
  without weakening the ratified `init` re-init guard (FR-028).
