...
- [ ] T001 Capture the FR-006 baseline: run `uv run pytest tests/e2e/test_research_workflow.py -q` and note it green BEFORE any fixture edit, so the later gate (T015) proves the M4 `factual_anchor` `{error:1, warning:1}` counts are byte-stable across this iteration's fixture extension.
