...
- **Skeleton `bible/locations/`**: as of commit `512fdd5` the `init` skeleton ships all three entity directories — `bible/characters/`, `bible/settings/`, and `bible/locations/` (each `.gitkeep`'d) — matching the existing `location.md.tmpl` mold. (This restored the symmetry an earlier draft of this spec had flagged as a pending iteration-4 hygiene fix.) `bookwright-bible` still ensures these directories exist before stamping (FR-019), but as a **robustness measure, not compensation for a known skeleton gap**: a project may have been initialized by an older skeleton or had a directory removed, and the command must not fail on a missing entity directory.
