...
- [X] T001 Confirm the sweep baseline matches research.md before editing: run
...
- [X] T002 Create the `tests/meta/` package: add `tests/meta/__init__.py`
...
- [X] T003 Re-verify the comment-only precondition (FR-008, Assumptions): for
...
- [X] T004 [P] [US1] Edit the two `src/` hits per research.md `### src/`:
...
- [X] T005 [P] [US1] Edit `tests/commands/` hits per research.md
...
- [X] T006 [P] [US1] Edit `tests/core/` hits per research.md `### tests/core`
...
- [X] T012b [P] [US1] Edit the two top-level `tests/` harness hits per
...
- [X] T013 [US1] Run the Phase-1 sweep again across `src/` + `tests/`; confirm
...
- [X] T014 [US2] Verify strip-token preservation: for every `S` row in
...
- [X] T015 [US2] Verify no durable number was renumbered or reworded anywhere
...
- [X] T016 [US2] Verify relabel / remove / neutral-prose rows: each `L`/`R`/`P`
...
- [X] T017 [US3] Implement the gate at
...
- [X] T018 [US3] Run the gate alone and confirm green on the cleaned tree:
  `uv run pytest tests/meta/test_no_traceability_tags.py -q` (contract C1,
  US3-AS1).
- [X] T019 [US3] Prove the gate bites (contract C2, US3-AS2): inject a probe
...
- [X] T020 [US3] Confirm no false positives (contract C4–C5, FR-011): the gate
...
- [X] T021 Confirm comment-only edits (FR-008, quickstart §2): run
...
- [X] T022 Confirm `specs/` untouched (FR-012, SC-005): `git status specs/`
  shows no modified files in this iteration's working tree beyond the spec-kit
  artifacts authored before implementation.
- [X] T023 Run the full quality gate (SC-003, quickstart §5): `uv run pytest`
