...
- [X] T010 [P] [US1] Add the **registry well-formedness** test: assert `set(DEFERRED_CONCEPTS) <= set(CONCEPTS)`, `len(DEFERRED_CONCEPTS) == 7`, the key set equals the seven names, every `reason` is non-empty, and no carrier name (`CharacterFeature`, `Dimension`, `Type`, `TimeInterval`) appears. **Pin the full concept→`target_version` mapping** by asserting `{name: note.target_version for name, note in DEFERRED_CONCEPTS.items()}` equals the exact expected dict (`NarrativeLocation`/`Object`→`"v0.3.x"`, `NarrativeUnit`/`NarrativeFunction`/`NarrativeSequence`→`"v0.4"`, `RelationshipRole`/`PsychologicalState`→`"undecided"`) — a wrong version must fail, so the value is a contract, not a comment (FR-002, FR-010, SC-002, contracts §1).
- [X] T011 [US1] Define the pure helper `parity_diff(reachable: set[str], deferred: set[str]) -> tuple[set[str], set[str]]` returning `(fed_but_deferred = reachable & deferred, undeclared_orphans = (set(CONCEPTS) - reachable) - deferred)` in `tests/golem/test_ingestion_parity.py`, with a docstring tying it to FR-006/007/008 (data-model.md §Failure-message contract, research D5).
- [X] T012 [US1] Add the **live guard** test: assert `orphans == set(DEFERRED_CONCEPTS)`; on failure, call `parity_diff(reachable, set(DEFERRED_CONCEPTS))` and include both returned sets in the assertion message so a real drift names the offending concept(s) (FR-005, SC-001, contracts §2).
- [X] T013 [P] [US1] Add the three **drift-simulation** tests driving `parity_diff` on perturbed *local copies* (never mutating `DEFERRED_CONCEPTS`): (a) a reachable concept added to the deferred copy → `fed_but_deferred` names it (FR-006); (b) the same `reachable ∩ deferred ≠ ∅` condition viewed from the registry side, i.e. declaring an already-fed concept as deferred (FR-007) — (a) and (b) intentionally exercise the one shared set condition from both narrative directions, not two distinct mechanisms; (c) a real orphan dropped from the deferred copy → `undeclared_orphans` names it (FR-008). Assert each named concept appears in the corresponding returned set (SC-003, contracts §3, research D5).
- [X] T014 [P] [US2] Add the **determinism** test: perform a **second, independent** `build_project_graph(parity_exercise_root, manifest)` (a fresh build, not a re-query of the module-scoped engine) and re-derive `reachable`/`orphans` from it, then assert the two verdicts are identical — this exercises FR-009's "identical inputs yield an identical verdict across runs" at the build level, not merely the query level (the probe is `DISTINCT ?t`, set-valued/order-independent, and `parity_diff` is a pure set function) (FR-009, SC-004, contracts §2).
...
- [X] T015 [P] [US3] Extend the module docstring of [src/bookwright/io/manuscript.py](../../src/bookwright/io/manuscript.py) (which already says v0 does no prose mining) so it explicitly states that **both** `manuscript/` and `outline/` are author-only in v0.3 — presence/scaffold only, engine does not ingest them — and points to the deferral rationale. Change docstring **only**; `manuscript_present` and all logic stay byte-identical (FR-011, SC-005, research D6). English (code convention).
- [X] T016 [P] [US3] Add one Spanish line to [docs/authoring.md](../../docs/authoring.md) stating that `outline/` and `manuscript/` are author-only in v0.3 (the scaffold creates them but the engine does not ingest them) (FR-011, SC-005, research D6). Spanish (docs convention, CLAUDE.md).
...
- [X] T017 Run the quickstart end to end (quickstart.md §1–§5): the registry one-liner (§3) prints the seven sorted keys; the fixture build (§2) shows the six IRIs and no orphan IRI; the parity test passes twice with identical verdict (§5/SC-004).
- [X] T018 Run the full suite to confirm behavior-neutrality (SC-005): `uv run pytest` stays green with the 023 orchestration E2E (`tests/e2e/test_orchestration_workflow.py`) and the closure tests (`test_frozen_ontology.py`, `test_namespaces.py`) **unchanged** — no class/property added to the frozen closure (SC-006, Principle X).
- [X] T019 Run the four CI gates green (SC-006): `uv run ruff check && uv run ruff format --check`, `uv run mypy --strict`, `uv run pytest` (≥80% coverage, single-sourced in `pyproject`).
