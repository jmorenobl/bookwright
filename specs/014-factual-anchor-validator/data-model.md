...
| V3 | under-reliable | `warning` | FR-008 — anchor has ≥ 1 supporting source **and** (best rating < threshold **or** none is rated); suppressed for a zero-source anchor (R1 reports it) | rating too low → `anchor '<a>' is backed only by sources below the minimum reliability '<min>'`; none rated → `anchor '<a>' is backed by sources but none carries a reliability rating (minimum required: '<min>')` | `(anchor, bw:promotes, finding)` |
