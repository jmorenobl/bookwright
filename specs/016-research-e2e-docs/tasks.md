...
- [X] T001 Create the `tests/fixtures/tiny-historical/` directory tree (empty dirs):
  `bible/`, `bible/characters/`, `bible/settings/`, `bible/research/`, `outline/`, `manuscript/`
  — per `contracts/fixture-layout.md`.
- [X] T002 [P] Author `tests/fixtures/tiny-historical/manifest.toml` per
...
- [X] T003 Confirm the strict reader's contract by reading `src/bookwright/io/research.py` and
...
- [X] T004 Confirm `factual_anchor` firing conditions by reading
...
- [X] T005 [P] [US1] Author `tests/fixtures/tiny-historical/bible/constitution.md` — short Spanish
  premise/voice/rules for the historical novel (mirror `tiny-novel/bible/constitution.md`).
- [X] T006 [P] [US1] Author `tests/fixtures/tiny-historical/bible/timeline.md` with **≥1 event
...
- [X] T007 [P] [US1] Author `tests/fixtures/tiny-historical/bible/characters/<slug>.md` (≥1
...
- [X] T008 [P] [US1] Author `tests/fixtures/tiny-historical/bible/settings/<slug>.md` (≥1 setting)
  consistent with the historical period.
- [X] T009 [P] [US1] Author the outline skeleton
...
- [X] T010 [US1] Author `tests/fixtures/tiny-historical/bible/research/sources.md` — several
  ...
  on T003.
- [X] T011 [US1] Author `tests/fixtures/tiny-historical/bible/research/<topic>.md` — Findings citing
...
- [X] T012 [P] [US1] Author `tests/fixtures/tiny-historical/bible/research/_index.md` — ≥1 open
  question + topic map (prose body, not indexed) (contracts/fixture-layout.md).
- [X] T013 [US1] Author `tests/fixtures/tiny-historical/manuscript/NN-<slug>.md` — a chapter whose
  ...
  Depends on T011.
- [X] T014 [US1] Author `tests/fixtures/tiny-historical/expected-findings.md` — the co-located oracle
...
- [X] T015 [US2] Create `tests/e2e/test_research_workflow.py` scaffold: an oracle-loader helper that
...
- [X] T016 [US2] **Group A** (deterministic flow) in `tests/e2e/test_research_workflow.py`:
...
- [X] T017 [US2] **Group B** (verify preconditions) in `tests/e2e/test_research_workflow.py`:
...
- [X] T018 [US2] **Group D** (fixture hygiene, E1 invariants) in
...
- [X] T019 [US3] **Group C / no-directory** in `tests/e2e/test_research_workflow.py`: copy
...
- [X] T020 [US3] **Group C / disabled-block** in `tests/e2e/test_research_workflow.py`: copy
...
- [X] T021 [P] [US4] Author `docs/research.md` (new): the five required topics — what research is in
...
- [X] T022 [P] [US4] Edit `docs/validation.md`: add the `factual_anchor` row to "Validadores
  integrados" (rules R1–R5, warning/error severities) (FR-016).
- [X] T023 [P] [US4] Edit `docs/authoring.md`: note `bookwright-research` and `bookwright-verify` in
  the skills reference (trigger on ES + EN prompts) (FR-016).
- [X] T024 [P] [US4] Author `docs/changelog.md` (new): a v0.2.0 entry describing the research &
  verification system, plus a retroactive v0.1.0 entry (FR-018).
- [X] T025 [US4] Edit `mkdocs.yml` `nav`: add "Investigación: research.md" and "Cambios:
...
- [X] T026 [P] Run `uv run pytest tests/e2e/test_research_workflow.py` — the new regression is green.
- [X] T027 Run the full `uv run pytest` — overall coverage ≥ 80 % (the single enforced gate) and
  confirm new M4 code is ≥ 85 % as a **report-only** quality target (no second `fail_under`)
  (FR-019/SC-005).
- [X] T028 [P] Run `uv run ruff check && uv run ruff format --check && uv run mypy --strict`
  (FR-020/SC-006).
- [X] T029 Run `uv run mkdocs build` — strict mode, **zero warnings** (FR-021/SC-006).
- [X] T030 Walk `specs/016-research-e2e-docs/quickstart.md` end-to-end (incl. the documented manual
