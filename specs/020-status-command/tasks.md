...
- [ ] T004 [P] Extend `src/bookwright/io/research.py` additively: add frozen `FindingIdentity` (`id`, `relpath`, `uri`) and `AnchorIdentity` (`promotes_id`, `constrains: str | None` — authored name / `"timeline"` / `None`, `relpath`, `uri`); `ResearchResult` gains `finding_identities: tuple[FindingIdentity, ...]` and `anchor_identities: tuple[AnchorIdentity, ...]` populated during mapping; existing fields, entity models, and emitted triples untouched (research.md D2, data-model.md §4)
...
- [ ] T015 [P] [US1] Unit tests in `tests/status/test_model.py`: payload shapes, `count == len(items)` invariant, deterministic ordering, fixed `counts` key order (`error`/`warning`/`info`), serialized output contains no URIs / timestamps / environment data (depends on T010)
