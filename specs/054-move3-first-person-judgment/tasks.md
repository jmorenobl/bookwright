...
- [X] T001 Confirm the green baseline: `uv run pytest -q` passes on the iteration branch before any edit (so later red is attributable to this slice).
- [X] T002 Measure the description headroom: `uv run python -c "from bookwright.integrations.descriptions import SKILL_DESCRIPTIONS as D; print(len(D['bookwright-continuity']))"` — record the value (expected **1000**); the folded edit MUST stay **≤ 1024** (quickstart §1).
...
- [X] T013 [P] [US2] In `tests/commands/test_status.py`, assert the new first-person `next_action` surfaces through the `--json` envelope on a `(focalization, pending_capability, first_person_recall)` state (contract `status-nudge.md` Verification).
