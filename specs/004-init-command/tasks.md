...
- [X] T001 Create `src/bookwright/resources/project/__init__.py` (empty marker file, makes the packaged template tree a subpackage discoverable via `importlib.resources.files("bookwright.resources.project")`).
- [X] T002 [P] Create `src/bookwright/resources/vocabularies/__init__.py` (empty marker file, makes the packaged vocabulary stubs a subpackage).
- [X] T003 [P] Create `tests/commands/__init__.py` (empty marker file, makes the new test subpackage discoverable by `pytest`).
...
## Phase 10: Bug fixes from /code-review (round 4)

**Purpose**: Close three atomicity / error-classification defects (Bucket A) that the round-4 `/code-review` (extra-high effort) found against the existing FR-030 (atomic-or-nothing) and FR-035 / contract §4 (structured error envelopes). No spec changes — every fix lands behavior already required by the merged spec. The fourth task hardens the test fixture that hid the first two bugs.

**Sequencing**: T047 first (it's the fixture that lets the regression tests in T048 + T049 actually detect orphan dirs); then T048 + T049 + T050 in any order (different production sites, different test blocks); the quality-gate re-run is folded into T035 / T046's existing scope — no separate task needed since these fixes change no behaviour beyond the documented FRs.

- [ ] T047 [B4] Fix the `dirhash` helper in [tests/commands/conftest.py:54](tests/commands/conftest.py#L54) so it captures empty directories in addition to files and symlinks. Current body only adds entries when `child.is_file() or child.is_symlink()` — so a pre-scaffold failure that leaves `mi-libro/` (empty) on disk passes the `dirhash(before) == dirhash(after)` assertion (false negative; this is the fixture defect that hid B2 from `test_pre_scaffold_failure_leaves_tree_unchanged`). Add a `<DIR>` sentinel digest for `child.is_dir()` entries (do NOT recurse the dir's contents in the digest — `rglob("*")` already enumerates children, so each subdirectory contributes its own `<DIR>` entry). **FR violated**: indirectly FR-030 / SC-005 (the fixture's blindness allowed regressions against atomic-or-nothing to merge silently). **Acceptance**: the parametrized `test_pre_scaffold_failure_leaves_tree_unchanged[…unknown_integration…]` case in [tests/commands/test_init_rollback.py:25-28](tests/commands/test_init_rollback.py#L25-L28) MUST fail before T049 lands (orphan `mi-libro/` dir is now visible in the diff) and pass after.

- [ ] T048 [B1] Close orphan-dir + uncaught `pydantic.ValidationError` on absolute `--skills-dir` paths. Reproduction: `bookwright init my-proj --integration generic --integration-options "--skills-dir /tmp/foo" --json` currently exits 1 with a Python traceback on stderr (no envelope) AND leaves `my-proj/` on disk. Root cause: `ResolvedInvocation(...)` construction at [src/bookwright/commands/init/main.py:173](src/bookwright/commands/init/main.py#L173) trips the `_check_skills_dir` field validator at [src/bookwright/commands/init/envelope.py:72-79](src/bookwright/commands/init/envelope.py#L72-L79), but the construction happens AFTER `project_root.mkdir(...)` at line 131 and is not wrapped in any handler. **Fix**: as part of the T049 reorder, wrap the `ResolvedInvocation(...)` call in `try/except pydantic.ValidationError` and surface the first error as `code: "malformed_option"`, `exit_code: 5`, `details: {"field": <loc>, "value": <input>, "rule": <msg>}` via `envelope.emit_error(rolled_back=False)`. **FRs violated**: FR-030 (atomic-or-nothing — orphan dir remains), FR-035 / contract §4 (uncaught exception escapes the structured-error contract). **Acceptance**: the reproduction above exits 5 with a single JSON envelope on stdout (`code: "malformed_option"`, `details.value` references `/tmp/foo`, `rolled_back: false`), nothing on stderr, and `my-proj/` does NOT exist on disk afterwards. Add the regression row to the `_VALIDATION_FAILURES` parametrize in [tests/commands/test_init_rollback.py:17](tests/commands/test_init_rollback.py#L17) so the fixed `dirhash` (T047) verifies the no-orphan-dir invariant.

- [ ] T049 [B2] Close orphan-dir on unknown-integration / option-parsing failure. Reproduction: `bookwright init my-proj --integration copilot --no-git --json` correctly returns exit 5 with `code: "unknown_integration"` and `rolled_back: false`, BUT leaves an empty `my-proj/` directory on disk. Root cause: `project_root.mkdir(parents=True, exist_ok=False)` at [src/bookwright/commands/init/main.py:131](src/bookwright/commands/init/main.py#L131) runs BEFORE `resolve.resolve_integration(...)` at line 153, so any failure between the two leaks the directory. **Fix**: reorder the orchestrator so that author / language / integration-class / parsed-options / skills-dir / `git_status` derivation AND `ResolvedInvocation(...)` construction all run BEFORE the mkdir. Concretely: hoist [main.py:151-189](src/bookwright/commands/init/main.py#L151-L189) above [main.py:129-149](src/bookwright/commands/init/main.py#L129-L149); pass `Path.cwd()` (not `project_root`) to `resolve_authors_or_warn` since `project_root` does not yet exist (git config walks upward, so the resolved name is unchanged). `cleanup_project_root = not project_root.exists()` is still captured at mode-resolution time so its semantics ("did `init` create this dir?") are preserved. **FRs violated**: FR-030 (atomic-or-nothing) and SC-005 (failure modes leave the target byte-for-byte unchanged). **Acceptance**: the reproduction above leaves no `my-proj/` directory; the parametrized `unknown_integration` row in [tests/commands/test_init_rollback.py:25-28](tests/commands/test_init_rollback.py#L25-L28) passes against the fixed `dirhash` (T047) without modification.

- [ ] T050 [B3] Close `MalformedOptionError` misclassified as `filesystem_error` when raised by `SkillsIntegration.setup()` at scaffold time. Reproduction: `bookwright init x --integration generic --integration-options "--skills-dir ." --json` currently exits 6 with `code: "filesystem_error"` and `details: {"path": "", "errno": 0}`, hiding the real rule (`resolves_to_project_root` from [src/bookwright/integrations/base.py:99](src/bookwright/integrations/base.py#L99)). Root cause: `classify_filesystem_failure` at [src/bookwright/commands/init/envelope.py:191](src/bookwright/commands/init/envelope.py#L191) has no branch for `MalformedOptionError`, so it falls through to the generic `OSError` / fallback path. **Fix**: add a top-of-cascade branch — when `exc` is a `bookwright.integrations.MalformedOptionError`, return `("malformed_option", 5, {"value": exc.value, "rule": exc.rule})` (matches the existing convention used by `resolve.resolve_integration` for parse-time `MalformedOptionError`). Import the exception at module top — the package restructure (T043) already eliminated the cycle that forced the existing lazy imports. **FR violated**: FR-035 / contract §4 (errors MUST surface with the contract-pinned `code` / `exit_code` / `details` shape; setup-time `resolves_to_project_root` and `escapes_project_root` are option-domain errors, not filesystem errors). **Acceptance**: the reproduction exits 5 with `code: "malformed_option"`, `details.rule == "resolves_to_project_root"`, `details.value == "."`, `rolled_back: true`, and `dirhash` is unchanged. Add the inline regression in [tests/commands/test_init_rollback.py](tests/commands/test_init_rollback.py) (cannot go in the existing `_VALIDATION_FAILURES` parametrize because that grid pins `rolled_back: false`).

**Checkpoint**: B1, B2, B3 closed against the spec-ratified FRs; B4 fixture hardened so future regressions in this area cannot silently pass. No FR / contract / data-model / plan changes. After T050 lands, re-run `uv run pytest tests/commands/ -x && uv run ruff check && uv run ruff format --check && uv run mypy --strict src tests` (same gate as T035 / T046) and confirm all green.
