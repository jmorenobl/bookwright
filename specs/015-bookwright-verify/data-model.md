...
| (b) violated anchor | `bw:Anchor` from the graph | the researched fact the passage breaks; a passage breaking N anchors lists all N (edge case) |
| (c) source / provenance | `bw:Source` behind the anchor | cited as the graph records it, including original-language references (foreign-source edge case) |
...
redefined by this iteration:

- **`bw:Anchor`** — the binding researched fact; promotes a `Finding`, constrains a
  narrative entity, backed by ≥ 1 `Source`. Loaded via `graph query`.
- **`bw:Source`** — provenance behind an anchor; cited in the report (part c).
- the **finding → anchor → source** chain iteration 012 emits, traversed by the
  agent's SPARQL to attach provenance to each contradiction.
