...
- [ ] T002 Add `"yaml"` to `RUNTIME_MODULES` in [src/bookwright/commands/check.py](src/bookwright/commands/check.py#L11) so `bookwright check` verifies the new runtime import.
...
- [ ] T030 [US3] Emit provenance in the build path — extend [src/bookwright/io/bible.py](src/bookwright/io/bible.py) (and the build wiring in [src/bookwright/commands/graph/build.py](src/bookwright/commands/graph/build.py) if needed) to construct one iteration-5 `AttributeAssignment` per derived attribute assertion (feature, role, birth/death, participation) plus a file-level assignment for the identity assertion, with `source = "<relpath>"` or `"<relpath>:<line>"` from `key_lines`, and feed its triples into the engine (FR-011, SC-006, data-model § 4, R6). (depends on T024, T025)
...
- [ ] T040 Confirm module sizes: every new module under `indexers/`, `io/`, `commands/graph/` is ≤ 500 lines (Principle IV); split if any approaches the ceiling. Also confirm the FR-017 read-only boundary: `commands/graph/` exposes no graph-mutation/write-back path beyond `build`'s `engine.save`, and neither verb imports a validator (semantic coherence is iteration 10).
