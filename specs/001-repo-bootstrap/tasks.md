...
- [ ] T001 Create the source-tree skeleton: empty directories `src/bookwright/`, `src/bookwright/commands/`, `tests/`, `.github/workflows/` (with a single `.gitkeep` in any directory that would otherwise be empty pre-implementation).
- [ ] T002 Author `pyproject.toml` at repo root with project metadata (`name = "bookwright-cli"`, `requires-python = ">=3.11"`, `dynamic = ["version"]`, `license = { text = "Apache-2.0" }`, classifiers), the full runtime dependency set from Constitución § Technical Constraints (`typer>=0.12`, `rich>=13.7`, `rdflib>=7.0`, `pydantic>=2.5`, `tomlkit>=0.12`, `jinja2>=3.1`, `python-slugify>=8.0`, `platformdirs>=4.2`, `uuid-utils>=0.16`), `[dependency-groups].dev` per research.md R1, `[project.scripts].bookwright = "bookwright.cli:app"`, and `[build-system]` block pinning `hatchling` per research.md R2.
...
- [ ] T005 [P] Add `[tool.pytest.ini_options]` and `[tool.coverage.run]` blocks to `pyproject.toml` per research.md R8 (`testpaths = ["tests"]`, `addopts = "-ra --cov=bookwright --cov-report=term-missing --cov-report=xml"`, `source = ["src/bookwright"]`, `branch = true`). No `--cov-fail-under` (FR-020 + spec Assumption).
- [ ] T006 [P] Add `[tool.hatch.build]` (`include = ["src/bookwright"]`) and `[tool.hatch.version]` (`path = "src/bookwright/__init__.py"`) blocks to `pyproject.toml` per research.md R2.
- [ ] T007 [P] Create `.python-version` at repo root with the single line `3.11`.
- [ ] T008 [P] Create `.gitignore` at repo root covering Python caches (`__pycache__/`, `*.pyc`), `.venv/`, `htmlcov/`, `coverage.xml`, `.coverage`, `.pytest_cache/`, `.ruff_cache/`, `.mypy_cache/`, `dist/`, `build/`, `*.egg-info/`.
- [ ] T009 [P] Create `LICENSE` at repo root with the full Apache-2.0 text per research.md R13.
...
- [ ] T011 Run `uv lock` to generate `uv.lock` at repo root, then commit it alongside `pyproject.toml` (FR-002, data-model.md E2). Depends on T002–T006 being complete in `pyproject.toml`.
...
- [ ] T012 Create `src/bookwright/__init__.py` with module docstring and the single source-of-truth line `__version__ = "0.0.1"` (data-model.md E1, research.md R11). Hatchling reads this at build-time.
...
### Tests for User Story 1 (FR-009c, FR-018)
...
- [ ] T019 [P] [US1] Write `tests/test_cli_version.py` — uses the `runner` fixture to invoke `bookwright version` and assert (a) exit code 0, (b) stdout in human mode contains the package version and the literal `unknown`; then invoke `bookwright version --json` and assert (a) exit code 0, (b) stdout parses as JSON with exactly the keys `package_version` and `golem_schema_version` (matching `contracts/version.schema.json`), (c) `golem_schema_version == "unknown"`, (d) nothing extra on stdout. Covers FR-009c for `version`.
...
**Checkpoint**: US1 acceptance scenarios 1–3 (spec.md) pass end-to-end. `uv run pytest tests/test_smoke_import.py tests/test_cli_version.py` is green. `uv run bookwright --help` lists `version`. This is the MVP cut.
...
- [ ] T024 [US4] Write `tests/test_cli_check.py` — invokes `bookwright check` and asserts (a) exit code 0 in the test environment, (b) stdout in human mode contains `OK` lines for each check; then invokes `bookwright check --json` and asserts (a) exit code 0, (b) stdout parses as JSON matching `contracts/check.schema.json` (validate keys: `ok` bool, `checks` non-empty list whose elements have `name`, `status ∈ {"ok","fail"}`, optional `detail`), (c) at least the `python_version` check is present, (d) `ok == True`. Also include a unit-level test that monkeypatches `importlib.import_module` to raise `ImportError` for one declared module and asserts that the corresponding `dependency:<name>` check yields `status: "fail"` with non-empty `detail`, and that the aggregate `ok` becomes `False` and the exit code is 1.

### Implementation for User Story 4

- [ ] T025 [US4] Implement `src/bookwright/commands/check.py` per research.md R12 and data-model.md P2: define a module-level constant tuple of declared runtime modules in resolution order (`"typer"`, `"rich"`, `"rdflib"`, `"pydantic"`, `"tomlkit"`, `"jinja2"`, `"slugify"`, `"platformdirs"`, `"uuid_utils"`); implement `run(json_output: bool = typer.Option(False, "--json"))` that (a) builds a list of `CheckResult`-shaped dicts starting with the `python_version` check (`sys.version_info >= (3, 11)`, `detail` always set to `f"{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}"` on OK, `f"found X.Y.Z, requires >=3.11"` on fail), then iterates the module tuple and appends `{"name": f"dependency:{m}", "status": "ok"}` on success or `{"name": ..., "status": "fail", "detail": str(exc)}` on `ImportError`; (b) computes `ok = all(c["status"] == "ok" for c in checks)`; (c) in `--json` mode emits `json.dumps({"ok": ok, "checks": checks}, separators=(",", ":"))` + newline to stdout and nothing else; (d) in human mode prints a `rich` table or list with one OK/FAIL line per check; (e) raises `typer.Exit(code=0 if ok else 1)`.
...
- **Phase 1 (Setup)**: No dependencies — can start immediately. T011 (`uv lock`) depends on T002–T006 being merged into `pyproject.toml`.
...
- T003 / T004 / T005 / T006 — all add disjoint top-level tables to `pyproject.toml` and are textually independent, but they all touch the same file, so they are parallel in planning but serialize on edit. Treat them as a single batch by one dev.
- T007 / T008 / T009 / T010 — disjoint files, fully parallel.
- T013 / T014 / T016 / T017 — disjoint files inside Phase 2, fully parallel.
- T018 / T019 inside US1 — disjoint test files, fully parallel.
...
```bash
# After T001 + T002 land, these can be authored in parallel by one dev or split across devs:
Task: "Create .python-version with the line '3.11'"                                   # T007
Task: "Create .gitignore covering Python caches, .venv, coverage, mypy/ruff caches"    # T008
Task: "Create LICENSE with the Apache-2.0 text"                                        # T009
...
```bash
# After T015 (cli.py skeleton) lands, the two test files are disjoint:
Task: "Write tests/test_smoke_import.py asserting bookwright.__version__ is set"
Task: "Write tests/test_cli_version.py covering human and --json modes"
```
