...
- **Cobertura de tests por debajo del umbral constitucional**: en este bootstrap la superficie de código es mínima (entry point + `version` + `check`); el harness de cobertura debe poder reportar el porcentaje aunque el umbral exigido por la Constitución (≥ 80 %) se trate como meta y no como blocker hasta que exista lógica de dominio.
...
- **FR-009a**: Tanto `bookwright version` como `bookwright check` MUST aceptar la bandera `--json`. Cuando se pasa, MUST emitir un único documento JSON en stdout (y solo eso); cualquier mensaje de progreso o ruido va a stderr. Sin la bandera, mantienen su salida humano-legible por defecto.
...
- **Cobertura como meta no bloqueante en M0**: la Constitución (Principio VIII) exige ≥ 80 % de cobertura. Esta iteración deja el harness listo y reporta el porcentaje, pero como la superficie de código es mínima y trivial, no se exige el umbral como gate bloqueante para esta PR específica; las iteraciones que introducen lógica de dominio sí lo harán.
