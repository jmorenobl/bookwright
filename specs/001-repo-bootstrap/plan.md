...
| VIII | Test Discipline | Aplica | ⚠️ Parcial justificado | pytest + ruff + mypy corren en CI sobre cada push/PR. Harness de cobertura instalado y reportando local + artefacto CI. El umbral ≥ 80 % **no** se exige como gate bloqueante en esta PR por instrucción explícita de la spec (Assumption final + Edge Case sobre cobertura), porque la superficie de código es trivial y el cálculo de cobertura sobre 200 LOC es ruidoso. Iteraciones que introducen lógica de dominio ya activan el umbral. |
